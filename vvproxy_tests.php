...
class vvproxy_tests {
	...
	function  vvproxy_tests() {

		$this->wsdl = 'http://10.21.0.200:8000/flexservices/services/vvws_tests.php?wsdl';
	...
	* @param string $username
	...
	function getMailSupport() {
		...
		$mailbox = 'mail.hdistribution.fr:993';
		$username = 'glpi';
		$password = 'glpiglpi';
		$encryption = 'ssl';
		$result = array();
		...
		array_push($result,$overallMessages);
		array_push($result,$unreadMessages);
		// 		fetch all messages in the current folder
						$emails = $imap->getMessages();
		array_push($result,$emails);
		return $result;
	...
	* @param string $username
	...
	function restartOrkarte() {
		require_once dirname(__FILE__) .  '/Net/SSH2.php';
		$ssh = new Net_SSH2('10.21.60.1');
	...
	*
		...
		$client = new Zend_Soap_Client($this->wsdl);
		$result = $client->ork_ca_mag($ip,$annee,$mois,$jour);
	...
	* @param array $tab
		...
		*/
		...
		// 		Redirect output to a clientâ€™s web browser (Excel2007)
								//h		eader('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		//h		eader('Content-Disposition: attachment;filename="01simple.xlsx"');
		//h		eader('Cache-Control: max-age=0');
	...
	*
		...
		fwrite($fh, new ByteArray($donnes));
		//n		ew ByteArray();
		fclose($fh);
		return true;
		//$		client = new Zend_Soap_Client($this->wsdl);
